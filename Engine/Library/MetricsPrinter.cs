...
public static class MetricsPrinter
{
    ...
    public static void PrintEntityMetrics(List<Entity> entities)
    {
        ...
        Console.WriteLine($"  Mean Waiting Time:        {(exited.Count > 0 ? exited.Average(e => e.TotalWaitingTime) : 0.0):F2}");
        Console.WriteLine($"  Mean Service Time:        {(exited.Count > 0 ? exited.Average(e => e.TotalServiceTime) : 0.0):F2}");
    }
